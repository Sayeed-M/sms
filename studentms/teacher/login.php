...
if (isset($_POST['login'])) {
    ...
    $password = $_POST['password']; // Use plaintext for comparison as the example password is not hashed

    $sql = "SELECT ID FROM teacher WHERE UserName=:username and Password=:password";
    $query = $dbh->prepare($sql);
    $query->bindParam(':username', $username, PDO::PARAM_STR);
    $query->bindParam(':password', $password, PDO::PARAM_STR); // Bind plaintext password
    $query->execute();
